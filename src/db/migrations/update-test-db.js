/**
 * Migration script to update the test database with visualization columns
 */
...
const dbPath = path.join(__dirname, '../../../data/test-user-data.db');
...
async function runMigration() {
  return new Promise((resolve, reject) => {
    ...
    const db = new sqlite3.Database(dbPath, (err) => {
      ...
      logger.info(`Connected to the test database at ${dbPath}`);
...
if (require.main === module) {
  runMigration()
    ...
    .catch((err) => {
      logger.error(`Test database migration failed: ${err.message}`);
      process.exit(1);
    });
}
